...
const names = ["jan", "Kees", "Arjan", "Rob", "Daan"];

for (let i = 0; i < names.length; i++) {
  const greeting = greeter(names[i]);
...
const minutes = [1, 3, 23]
for (let i = 0; i < minutes.length; i++) {
  const outcomeSeconds = minutesToSeconds(minutes[i]);
  console.log(outcomeSeconds);
}
//of//
...
function calculateDogYears(dogYear) {
  return "Jouw hond is " + (dogYear * 7) + " jaar oud in mensenjaren.";
}
...
function wrapper(word, teken) {
  return teken + word + teken;
}
